// Seed خودکار سیستم — در اولین لاگین اگر دیتابیس خالی باشد اجرا می‌شود
// تا دیپلوی Vercel + Supabase بدون اجرای دستی اسکریپت، آماده کار باشد
...
export async function seedDatabase(db: PrismaClient) {
  console.log("[seed] Seeding MEP-PMS demo data ...");
  ...
  await db.payment.createMany({
    data: [
      { tenantId: tenant.id, amount: 540000000, method: "TRANSFER", paidAt: new Date(Date.now() - 49 * 86400000), gatewayRef: "TRF-88121", note: "پیش‌پرداخت برج آرمان" },
      { tenantId: tenant.id, amount: 285000000, method: "GATEWAY", paidAt: new Date(Date.now() - 25 * 86400000), gatewayRef: "SHAPARAK-A1B2C", note: "پیش‌پرداخت مجتمع کیان" },
    ],
  });

  await db.guaranteeCheck.createMany({
    data: [
      { tenantId: tenant.id, personId: teamLeader1.id, checkNumber: "CHK-770121", amount: 200000000, bankName: "ملت", dueDate: new Date(Date.now() + 45 * 86400000), status: "VALID", notes: "چک ضمانت حسن انجام کار" },
      { tenantId: tenant.id, personId: teamLeader2.id, checkNumber: "CHK-880455", amount: 150000000, bankName: "صادرات", dueDate: new Date(Date.now() + 12 * 86400000), status: "VALID", notes: "نیازمند تمدید" },
    ],
  });
  ...
  console.log("[seed] Seed complete! Users: manager/secretary/supervisor/accountant/marketer/team — pass: 1234");
}
...
/**
 * اگر جدول User خالی باشد، داده‌های دمو را یک‌بار می‌سازد.
 * در صورت خطا (مثلاً نبود تیبل‌ها) قفل آزاد می‌شود تا تلاش بعدی ممکن باشد.
 */
export function ensureSeeded(db: PrismaClient = defaultDb): Promise<void> {
  if (!seedPromise) {
    seedPromise = (async () => {
